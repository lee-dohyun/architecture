...
# 아키텍처 데이터 (2026-08-15 최신화 — store.front 이름 변경, posselect-shell 추가, storybook 노출, Velero 복구 검증 등 반영)
architecture_data = {
    "current": {
        ...
        "services": {
            ...
            "shop_line": [
                {"name": "store.front", "domains": ["home.posselect.com"], "desc": "쇼핑몰 메인 랜딩 페이지 (Next.js, 2026-08-13 구 home.front에서 이름 변경)", "status": "deployed"},
                {"name": "customer.front / auth.api", "domains": ["customer.posselect.com"], "desc": "로그인/회원가입/정보수정. 2026-08-13 이용약관/개인정보방침 및 마케팅 수신동의 백엔드 저장 완료", "status": "deployed"},
                ...
                {"name": "posselect-ui", "domains": ["storybook.posselect.com", "ui.posselect.com (별칭)"], "desc": "디자인 시스템 레퍼런스 페이지. 2026-08-14 storybook 메인 도메인 적용", "status": "deployed"},
                {"name": "posselect-shell", "domains": ["shell.posselect.com"], "desc": "전 프론트가 <script>로 로드하는 공통 헤더/푸터 런타임 셸(nginx 정적 서빙)", "status": "deployed"}
            ],
            "mail": [
            ...
            "infrastructure": [
                ...
                {"name": "Velero (백업 자동화)", "desc": "2026-08-04 배포, 2026-08-13 1차 restore 리허설 완료(검증 통과). MinIO를 백업 스토리지로 사용, PVC 데이터 포함 전 네임스페이스 매일 백업", "status": "deployed"}
            ],
            "cicd": [
                {"name": "GitHub Actions CI (이미지 빌드+푸시)", "scope": "전 저장소", "status": "deployed"},
                {"name": "self-hosted runner 기반 CD (자동 배포)", "scope": "gateway / auth.api / store.front / customer.front / admin.front / order.api / product.api", "status": "deployed"},
                {"name": "수동 배포", "scope": "architecture-web, wordpress, tool 등", "status": "manual"}
            ]
        },
        "issues": [
            "보안 강화 미적용 (RBAC 세분화, NetworkPolicy, Istio mTLS — 계획 문서만 존재)",
            "분산 트레이싱은 Java 백엔드 4개만 계측됨(Next.js 프론트는 미계측), Kiali/서비스 메시 관찰성은 여전히 없음",
            ...
            "customer.front 간편 로그인(카카오/네이버/구글) 구현 및 휴대폰 본인인증 백엔드 연동 필요",
            "Velero 주기적(3개월 등) 복구 재검증 루틴 필요"
        ]
    },
    "domain_split": {
    ...
    "resources": {
        "cpu": "~5% (단일 6코어 Ryzen 5 1600 기준)",
        "memory": "~19% (62Gi DDR4 듀얼채널 기준)",
        "nodes": 1,
